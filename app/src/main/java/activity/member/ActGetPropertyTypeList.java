...
public class ActGetPropertyTypeList extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_member_get_property_type_list);
        ButterKnife.bind(this);
        initialize();
    }

    private void initialize() {
        lblLayout.setText("MemberPropertyTypeActList");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        getSupportActionBar().setTitle("MemberPropertyTypeActList");
    ...
    private void getData() {
        MemberPropertyTypeRequest request = new MemberPropertyTypeRequest();
        ...
        RetrofitManager manager = new RetrofitManager(ActGetPropertyTypeList.this);
        IMember iMember = manager.getRetrofit(configStaticValue.GetApiBaseUrl()).create(IMember.class);
        Map<String, String> headers = new HashMap<>();
        headers = configRestHeader.GetHeaders(this);

        Observable<MemberPropertyTypeResponse> call = iMember.GetPropertyTypeActList(headers, request);
        call.observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Observer<MemberPropertyTypeResponse>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                    }

                    @Override
                    public void onNext(MemberPropertyTypeResponse response) {
                        JsonDialog cdd = new JsonDialog(ActGetPropertyTypeList.this, response);
                    ...
                    public void onError(Throwable e) {
                        ...
                        Toast.makeText(ActGetPropertyTypeList.this, "Error : " + e.getMessage(), Toast.LENGTH_LONG).show();
                    }
